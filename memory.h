...
#define HUGE_PAGE_SIZE (1 << HUGE_PAGE_BITS) // 2_097_152 = 2MiB page_size = 2MB
#define SIZE_PKT_BUF_HEADROOM 40

struct pkt_buf {
	
	uintptr_t buf_addr_phy;                // physical address of buffer block
...
struct mempool {                      
	void* base_addr;                  
	uint32_t buf_size;                
	uint32_t num_entries;             //number of memory block
